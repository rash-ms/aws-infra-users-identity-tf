locals {
  ...
  # Mapping group policies dynamically for all environments
  ...
  # Ensuring groups are mapped to emails and their policies
  group_mappings = {
  ...
  # Selecting policies for the given environment
  selected_policies = local.aws_policies[var.environment]

  # Define permission sets dynamically for all environments
  permission_sets = {
...
# Output debugging (useful for checking mappings)
...
# Attach permission sets to groups
...
# Create AWS Organizations Accounts (ensuring email format is valid)
